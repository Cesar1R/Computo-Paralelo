                                    ...
                                    int M)
{
   ...
   float local_sum = 0.0; 
   if(idx < N && idy < M){
      local_sum = (s_A[local_idx][local_idy] - s_B[local_idx][local_idy]) * (s_A[local_idx][local_idy] - s_B[local_idx][local_idy]);
   }
...
int main(void){
   ...
   //---------------------Inicio: Implementacion con memoria global 
    ...
    matrix_error_Shared<<<blocksPerGrid, threadsPerBlock>>>(d_A, d_B, d_sum, N_SIZE, M_SIZE); 
